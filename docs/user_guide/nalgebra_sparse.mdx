...
COO (Coordinate) is best used for constructing matrices, which can then be converted to CSR/CSC formats.
CSR (Compressed Sparse Row) is best used when you need fast *row* access.
CSC (Compressed Sparse Column) is best used when you need fast *column* access.
